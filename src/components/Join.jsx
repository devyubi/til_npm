...
const JoinSchema = yup.object({
  ...
  adress1: yup.string().required("우편번호는 필수 항목입니다"),
  adress2: yup.string().required("상세주소는 필수 항목입니다"),
});
      ...
      <form onSubmit={handleSubmit(onSubmitJoin)}>
        ...
        <div>
          ...
          {errors.userpw?.message && (
            <span style={{ color: "red" }}>{errors.userpw.message}</span>
          )}
        </div>
        <div>
          <label>전화번호</label>
          <input
            type="text"
            {...register("usernumber")}
            onChange={e => handdleChangePhone(e)}
          />
          {errors.usernumber?.message && (
            <span style={{ color: "red" }}>{errors.usernumber.message}</span>
          )}
        </div>
        <div>
          <label>도로명 주소</label>
          ...
          <button type="button" onClick={handleClickZipCode}>
            도로명 주소 검색
          </button>
        </div>
        <div>
