  ...
  return (
    <div>
      <form>
        ...
        <textarea></textarea>
        <br />
        {/* reactQuill은 textarea의 대체제임. css가 들어가야 모양이 나옴 (Form이 아님) */}
        <ReactQuill />
        <button>확인</button>
      </form>
    </div>
...
- html이 입력이 되는 것은 `크로스 사이트 스크립트 공격` 가능성이 있음
- 줄여서 `XSS 위험`이 존재함
- 위의 공격을 회피하기 위하여 추가 npm 설치가 필요함
- https://www.npmjs.com/package/dompurify
- `npm i dompurify`

- AddForm.jsx
- `<p dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(data) }}></p>` 쓰기
...
const AddForm = () => {
  ...
  return (
    <div>
      ...
      <div>
        <p>입력중인 내용 : {data}</p>
        {/* 아래처럼 출력해 봄 : React 에서 위험한 내용이고, html 형태인 것을 설정함 */}
...
- modules : "각 기능을 카테고리화 해서 묶어주는 기능"
  ...
  return (
    <div>
      <form style={{ width: "80%", height: 500 }}>
        <ReactQuill
          ref={quillRef} // 이미지 처리 2. 참조한 것을 연결함
          modules={modules}
          formats={formats}
          onChange={e => setData(e)}
        />
